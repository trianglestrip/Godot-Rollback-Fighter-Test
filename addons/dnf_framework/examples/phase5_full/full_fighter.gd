...
const COLOR_MAP := {
	DNFStates.State.IDLE: Color(0.2, 0.5, 0.8),
	DNFStates.State.WALK: Color(0.3, 0.6, 0.8),
	DNFStates.State.RUN: Color(0.15, 0.35, 0.9),
	DNFStates.State.JUMP: Color(0.4, 0.7, 0.9),
	DNFStates.State.FALL: Color(0.35, 0.65, 0.85),
	DNFStates.State.LAND: Color(0.25, 0.55, 0.75),
	DNFStates.State.DASH: Color(0.1, 0.3, 0.95),
	DNFStates.State.ATTACK: Color(0.9, 0.3, 0.2),
	DNFStates.State.HIT_STUN: Color(0.95, 0.85, 0.15),
	DNFStates.State.KNOCK_BACK: Color(0.85, 0.55, 0.1),
	DNFStates.State.KNOCK_DOWN: Color(0.6, 0.2, 0.1),
	DNFStates.State.AIR_BORNE: Color(1.0, 0.65, 0.2),
	DNFStates.State.GET_UP: Color(0.55, 0.55, 0.55),
	DNFStates.State.BLOCK: Color(0.3, 0.3, 0.7),
}
...
func _ready() -> void:
	...
	if hitbox:
		hitbox.deactivate()
		hitbox.add_to_group("dnf_hitbox")
	var hurtbox := get_node_or_null("Hurtbox")
	if hurtbox:
		hurtbox.add_to_group("dnf_hurtbox")
...
func _is_run_held() -> bool:
	...
	return abs(dir) > 0.01 and Input.is_action_pressed(input_prefix + "down")


func _is_dash_pressed() -> bool:
	return Input.is_action_just_pressed(input_prefix + "bomb")


func _get_input_dict() -> Dictionary:
	return {
		"punch": Input.is_action_just_pressed(input_prefix + "punch"),
		"kick": Input.is_action_just_pressed(input_prefix + "kick"),
	}
...
func _sync_hitbox_direction() -> void:
	if not hitbox:
		return
	var hb_shape := hitbox.get_node_or_null("HitboxShape") as CollisionShape2D
	if hb_shape:
		hb_shape.position.x = abs(hb_shape.position.x) * (1.0 if facing_right else -1.0)
...
func _update_facing_sprite() -> void:
	if body_rect:
		body_rect.scale.x = 1.0 if facing_right else -1.0


func _update_visuals() -> void:
	if body_rect:
		body_rect.color = COLOR_MAP.get(current_state, Color(0.5, 0.5, 0.5))
